Feature: relation link between ADR files
  ...
  Scenario Outline: relations between asciidoc adr files

    Given we have a clean system
      And there are the following adrs in the system
        | 0001-new-adr.adoc  | accepted    | 1   | New adr |
        | 0002-new-adr.adoc  | accepted    | 2   | New adr |
        | 0003-new-adr.adoc  | accepted    | 3   | New adr |
        | 0004-new-adr.adoc  | accepted    | 4   | New adr |

      And there is a config file created with this configuration
        | default_status | directory   | template_file           | id_digit_number |
        | custom         | ./          | ./template.adoc         | 4               |
    When the user specify the <title> title
      And the user specify the <relation> relation with the target ADR with the <targetADR> id
      And the create command is executed
    Then the <filename> ADR file is created
    ...
    Examples:
      | title    | filename           | relation     | targetADR | expectedStatus   | expectedTargetStatus |
      | New adr  | 0005-new-adr.adoc  | amend        | 2         | custom           | amended              |
      | New adr  | 0005-new-adr.adoc  | supersede    | 2         | custom           | superseded           |
      | New adr  | 0005-new-adr.adoc  | amend        | 3         | custom           | amended              |
      | New adr  | 0005-new-adr.adoc  | amend        | 4         | custom           | amended              |
